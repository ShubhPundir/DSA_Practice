...
bool singlesRemaining(const int *arr, int N){
    if(find(arr, arr + N, -1) == arr + N){
        return false; // -1 not found, hence no person in the array is single
    }
    return true; 
}
...
void solveSM(vector<int> *M, vector<int> *W, int N){
    ...
    for(int i = 0; i < N; i++){
        WE[i] = -1; ME[i] = -1;
    } // everybody is free or single

    while(singlesRemaining(WE,N)){ // could pass either WE or ME in the start
        for(int i = 0; i < N; i++){
